...
<div class="container mt-4">
    ...
    <div class="border-bottom mb-4 pb-2">
        <h3 class="mb-3">Formulir Edit Laporan Kemajuan Penelitian</h3>
        <p class="text-muted">Perbarui data laporan kemajuan penelitian dengan lengkap dan benar.</p>
    </div>
        ...
        <div class="card-body">
            <form action="{{ route('laporan_kemajuan_penelitian.update', $laporan_kemajuan_penelitian->id_laporan) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-sm-10">
                        <select name="skema" class="form-control @error('skema') is-invalid @enderror" required>
                            ...
                            @foreach ($skemas as $skema)
                                <option value="{{ $skema->id }}" {{ old('skema', $laporan_kemajuan_penelitian->skema_id) == $skema->id ? 'selected' : '' }}>
                    ...
                    <div class="col-sm-10">
                        <select name="jurusan" class="form-control @error('jurusan') is-invalid @enderror" required>
                            ...
                            @foreach ($jurusans as $jurusan)
                                <option value="{{ $jurusan->id }}" {{ old('jurusan', $laporan_kemajuan_penelitian->jurusan_id) == $jurusan->id ? 'selected' : '' }}>
                                    {{ $jurusan->jurusan }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-sm-10">
                        <select name="prodi" class="form-control @error('prodi') is-invalid @enderror" required>
                            ...
                            @foreach ($prodis as $prodi)
                                <option value="{{ $prodi->id }}" {{ old('prodi', $laporan_kemajuan_penelitian->prodi_id) == $prodi->id ? 'selected' : '' }}>
                                    {{ $prodi->prodi }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <select name="periode_laporan" class="form-control @error('periode_laporan') is-invalid @enderror" required>
                            ...
                            @for ($i = 1; $i <= 8; $i++)
                                <option value="semester {{ $i }}" {{ old('periode_laporan', $laporan_kemajuan_penelitian->periode_laporan) == 'semester '.$i ? 'selected' : '' }}>
                                    Semester {{ $i }}
                                </option>
                            @endfor
                        </select>
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <textarea name="ringkasan" class="form-control @error('ringkasan') is-invalid @enderror" rows="4">{{ old('ringkasan', $laporan_kemajuan_penelitian->ringkasan) }}</textarea>
                        @error('ringkasan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
