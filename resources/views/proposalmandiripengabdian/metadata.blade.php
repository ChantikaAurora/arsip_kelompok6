...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h3 class="mb-0 fw-semibold">Metadata Proposal Mandiri Pengabdian</h3>
                <p class="text-muted mb-0">Rekapitulasi data dari proposal Mandiri Pengabdian yang tercatat.</p>
            </div>
        </div>
    </div>
    ...
    <div class="d-flex justify-content-between mb-3">
        <div class="d-flex gap-2">
            {{-- Tombol Kembali --}}
            <a href="{{ route('proposal_mandiri_pengabdian.index') }}" class="btn btn-secondary shadow-sm d-inline-flex align-items-center">
                <i class="icon-action-undo align-middle mr-1"></i>Kembali
            </a>

            {{-- Tombol Unduh --}}
            <a href="{{ route('proposal_mandiri_pengabdian.metadata.download', ['search' => request('search')]) }}" class="btn btn-success shadow-sm d-inline-flex align-items-center" style="margin-left: 0.5rem;">
    ...
    <div class="d-flex justify-content-end mb-3">
        <form method="GET" action="{{ route('proposal_mandiri_pengabdian.metadata') }}" class="d-flex">
            ...
            <tbody>
                @forelse ($data as $i => $item)
                    <tr class="text-center align-middle">
                        ...
                        <td style="white-space: normal; word-wrap: break-word; max-width: 150px;">{{ $item->skemaPengabdian->skema_pengabdian ?? '-' }}</td>
