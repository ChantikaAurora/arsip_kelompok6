...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h3 class="mb-0 fw-semibold">Manajemen Proposal Mandiri Pengabdian</h3>
                <p class="text-muted mb-0">Kelola data proposal pengabdian yang diajukan secara mandiri.</p>
            </div>
        </div>
    </div>
    ...
    <div class="d-flex justify-content-between mb-3">
        <div class="d-flex gap-2">
            <a href="{{ route('proposal_mandiri_pengabdian.create') }}" class="btn btn-primary d-inline-flex align-items-center">
                <i class="icon-plus me-1 align-middle mr-1"></i> Tambah
            </a>
            <a href="{{ route('proposal_mandiri_pengabdian.metadata') }}" class="btn btn-success d-inline-flex align-items-center ms-2" style="margin-left: 0.5rem;">
                ...
                @forelse ($proposals as $proposal)
                    <tr>
                        ...
                        <td class="text-center align-middle">
                            <a href="{{ route('proposal_mandiri_pengabdian.show', $proposal->id) }}" class="btn btn-sm btn-primary me-1 mb-1">
                                <i class="bi bi-eye"></i> Detail
                            </a>
                            <a href="{{ route('proposal_mandiri_pengabdian.edit', $proposal->id) }}" class="btn btn-sm btn-warning me-1 mb-1">
                                <i class="bi bi-pencil-square"></i> Edit
                            </a>
                            <form id="deleteForm-{{ $proposal->id }}" action="{{ route('proposal_mandiri_pengabdian.destroy', $proposal->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
